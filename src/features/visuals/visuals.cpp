...
static void DrawAASideArrows(int aa_side, ImColor aa_color)
{
	Vector2D screen;
	...
	const auto yaw = DEG2RAD(aa_side ? 0 : 180);
	...
	if (aa_side)
	{
		...
		g_pRenderer->AddTriangleFilled(points[0], points[1], points[2], aa_color);
	}
	else
	{
		ImVec2 points[] =
...
void CVisuals::Overlay()
{
	...
	if (!g_Local->m_bIsDead && cvars::visuals.esp_other_local_aa_side_arrows && cvars::ragebot.active && cvars::ragebot.aa_enabled && cvars::ragebot.aa_stand_desync && !g_pGlobals->m_flGaitMovement)
	{
		...
		{
			DrawAASideArrows(0, ImColor(100, 255, 100));
			DrawAASideArrows(1, ImColor(100, 100, 100));
		}
		else {
			DrawAASideArrows(0, ImColor(100, 100, 100));
			DrawAASideArrows(1, ImColor(100, 255, 100));
		}
	}
}
