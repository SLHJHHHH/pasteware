...
void CKnifeBot::Run(usercmd_s* cmd)
{
	...
	if (flDistance <= 16.f)
	{
		DebugPrint("distance_too_small", 0.f, flDistance);
		return;
	}

	DebugPrint("running", 0.f, flDistance);
	...
	for (int i = 1; i <= client_state->maxclients; i++)
	{
		...
		{
			DebugPrint("fov", flFOV, flDistance, i);
			continue;
		}
		...
		if (cvars::misc.kb_aim_type && !hitboxes.empty())
		{
			...
			int iClosestTraceIndex = -1;
			float flClosestFraction = 1.f;
			...
			for (auto hitbox : hitboxes)
			{
				for (auto point = 0; point < HITBOX_POINTS_MAX; point++)
				{
					...
					const int iTraceIndex = g_Engine.pEventAPI->EV_IndexFromTrace(&pmTrace);

					if (iTraceIndex == g_Player[i]->m_iEntIndex && pmTrace.fraction < flBestFraction)
					{
						QBestAimAngles = QAimAngles;
						flBestFraction = pmTrace.fraction;
					}

					if (pmTrace.fraction < flClosestFraction)
					{
						iClosestTraceIndex = iTraceIndex;
						flClosestFraction = pmTrace.fraction;
					}
				}
			}
			...
			if (flBestFraction < 1.f)
			{
				...
				DebugPrint("aim_trace_miss", flFOV, flDistance, i, iClosestTraceIndex);
		}
		else
		{
			pmtrace_t pmTrace;
			...
			if (iTraceIndex == g_Player[i]->m_iEntIndex)
				bAttack = true;
			else
				DebugPrint("trace_miss", flFOV, flDistance, i, iTraceIndex);
		}
		...
		if (bAttack)
		{
			...
			if (!bBlock)
			{
				DebugPrint("attack", flFOV, flDistance, i, g_Player[i]->m_iEntIndex);

				cmd->buttons |= (cvars::misc.kb_attack_type == 0) ? IN_ATTACK : IN_ATTACK2;
				return;
			}

			DebugPrint("shield_block", flFOV, flDistance, i, g_Player[i]->m_iEntIndex);
		}
	}
}

void CKnifeBot::DebugPrint(const char* pszStage, const float& flFOV, const float& flDistance, const int& iPlayer, const int& iTraceIndex)
{
	if (!cvars::misc.kb_debug)
		return;
	...
	if (!bEvent)
	{
		if (client_state->time - dbLastPrint < 0.5)
			return;

		dbLastPrint = client_state->time;
	}

	if (bEvent)
		dbLastPrint = client_state->time;

	Utils::TraceLog("[knifebot] stage=%s weapon=%d clip=%d primary=%.3f secondary=%.3f next_attack=%.3f in_reload=%d | distance=%.1f fov=%.1f max_fov=%.1f | aim_type=%d attack_type=%d | player=%d trace_index=%d entity_index=%d\n",
		pszStage, g_Weapon->m_iWeaponID, g_Weapon->m_iClip, g_Weapon->m_flNextPrimaryAttack, g_Weapon->m_flNextSecondaryAttack, g_Weapon->m_flNextAttack, g_Weapon->m_fInReload,
		flDistance, flFOV, cvars::misc.kb_fov, cvars::misc.kb_aim_type, cvars::misc.kb_attack_type,
		iPlayer, iTraceIndex, (iPlayer >= 1 && iPlayer <= MAX_CLIENTS) ? g_Player[iPlayer]->m_iEntIndex : -1);
}
