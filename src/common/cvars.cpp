...
static void FileRageHandle()
{
	...
	g_pSettings->Key("rage.aim.key", cvars::ragebot.raim_key);
	...
	g_pSettings->Key("rage.aim.tapping_mode", cvars::ragebot.raim_tapping_mode); g_pSettings->Key("rage.aim.hitchance.seeds", cvars::ragebot.raim_hitchance_seeds, 128); g_pSettings->Key("rage.aim.reaim_bypass", cvars::ragebot.raim_reaim_bypass); g_pSettings->Key("rage.aim.force_min_damage.key", cvars::ragebot.raim_force_min_damage_key); g_pSettings->Key("rage.aim.force_min_damage.value", cvars::ragebot.raim_force_min_damage_value, 1);
	...
	for (int weapon_idx = 1; weapon_idx < WEAPON_MAX_COUNT; weapon_idx++)
	{
		...
		g_pSettings->Key(wpn + ".aim.automatic.penetration.minimum_damage", cvars::weapons[weapon_idx].raim_auto_penetration_min_damage); g_pSettings->Key(wpn + ".aim.hitchance.seeds", cvars::weapons[weapon_idx].raim_hitchance_seeds, 128); g_pSettings->Key(wpn + ".aim.force_min_damage", cvars::weapons[weapon_idx].raim_force_min_damage);
...
static void FileLegitHandle()
{
	...
	g_pSettings->Key("legit.aim.smoke_check", cvars::legitbot.aim_smoke_check);
	g_pSettings->Key("legit.aim.flashed_check", cvars::legitbot.aim_flashed_check, 50);
	g_pSettings->Key("legit.aim.shoot_through_teammates", cvars::legitbot.aim_shoot_through_teammates);
	g_pSettings->Key("legit.aim.pseudo_professional", cvars::legitbot.aim_pseudo_professional);
	g_pSettings->Key("legit.aim.write_demo_visible_only", cvars::legitbot.aim_write_demo_visible_only, true);
	g_pSettings->Key("legit.aim.mouse_event", cvars::legitbot.aim_mouse_event);
	g_pSettings->Key("legit.aim.fov_scale_by_fps", cvars::legitbot.aim_fov_scale_by_fps);
	g_pSettings->Key("legit.aim.dynamic_fov", cvars::legitbot.aim_dynamic_fov);
	g_pSettings->Key("legit.target.aim_selection", cvars::legitbot.target_aim_selection);
	g_pSettings->Key("legit.target.weight.head", cvars::legitbot.target_weight_head, 50);
	g_pSettings->Key("legit.target.weight.neck", cvars::legitbot.target_weight_neck, 40);
	g_pSettings->Key("legit.target.weight.chest", cvars::legitbot.target_weight_chest, 30);
	g_pSettings->Key("legit.target.weight.stomach", cvars::legitbot.target_weight_stomach, 20);
	g_pSettings->Key("legit.target.weight.arms", cvars::legitbot.target_weight_arms, 10);
	g_pSettings->Key("legit.target.weight.legs", cvars::legitbot.target_weight_legs, 10);
	g_pSettings->Key("legit.aim.recoil.return_angles", cvars::legitbot.aim_recoil_return_angles, true);
	g_pSettings->Key("legit.aim.recoil.no_mousemove_trigger", cvars::legitbot.aim_recoil_no_mousemove_trigger);
	g_pSettings->Key("legit.aim.recoil.disable_smooth_auto", cvars::legitbot.aim_recoil_disable_smooth_auto);
	...
	g_pSettings->Key("legit.trigger.dynamic", cvars::legitbot.trigger_dynamic);
	g_pSettings->Key("legit.trigger.turn_off_after_first_bullet", cvars::legitbot.trigger_turn_off_after_first_bullet);
	g_pSettings->Key("legit.trigger.shot_delay", cvars::legitbot.trigger_shot_delay, 120);
	g_pSettings->Key("legit.trigger.hitbox_scale.head", cvars::legitbot.trigger_hitbox_scale[0], 100.f);
	g_pSettings->Key("legit.trigger.hitbox_scale.neck", cvars::legitbot.trigger_hitbox_scale[1], 100.f);
	g_pSettings->Key("legit.trigger.hitbox_scale.chest", cvars::legitbot.trigger_hitbox_scale[2], 100.f);
	g_pSettings->Key("legit.trigger.hitbox_scale.stomach", cvars::legitbot.trigger_hitbox_scale[3], 100.f);
	g_pSettings->Key("legit.trigger.hitbox_scale.arms", cvars::legitbot.trigger_hitbox_scale[4], 100.f);
	g_pSettings->Key("legit.trigger.hitbox_scale.legs", cvars::legitbot.trigger_hitbox_scale[5], 100.f);
...
g_pSettings->Key("remove.visual_recoil", cvars::visuals.remove_visual_recoil);
g_pSettings->Key("remove.smoke", cvars::visuals.remove_smoke);
g_pSettings->Key("remove.screenshake", cvars::visuals.remove_screenshake);
g_pSettings->Key("remove.events", cvars::visuals.remove_events);
g_pSettings->Key("other.watermark", cvars::visuals.watermark);
	g_pSettings->Key("other.watermark.credits", cvars::visuals.watermark_credits, true);
g_pSettings->Key("other.watermark.time", cvars::visuals.watermark_elements[0], true);
g_pSettings->Key("other.watermark.name", cvars::visuals.watermark_elements[1], true);
g_pSettings->Key("other.watermark.framerate", cvars::visuals.watermark_elements[2], true);
g_pSettings->Key("other.watermark.latency", cvars::visuals.watermark_elements[3], true);
g_pSettings->Key("other.watermark.elements4", cvars::visuals.watermark_elements[4]);
g_pSettings->Key("other.watermark.elements5", cvars::visuals.watermark_elements[5]);
g_pSettings->Key("other.watermark.elements6", cvars::visuals.watermark_elements[6]);
g_pSettings->Key("other.watermark.elements7", cvars::visuals.watermark_elements[7]);
g_pSettings->Key("other.shared_esp", cvars::visuals.shared_esp);
g_pSettings->Key("other.crosshair.enabled", cvars::visuals.crosshair_enabled);
g_pSettings->Key("other.crosshair.dot", cvars::visuals.crosshair_dot);
g_pSettings->Key("other.crosshair.size", cvars::visuals.crosshair_size, 10);
g_pSettings->Key("other.crosshair.gap", cvars::visuals.crosshair_gap, 5);
g_pSettings->Key("other.crosshair.thickness", cvars::visuals.crosshair_thickness, 1);
g_pSettings->Key("other.crosshair.dynamic", cvars::visuals.crosshair_dynamic);
g_pSettings->Key("other.crosshair.dynamic_scale", cvars::visuals.crosshair_dynamic_scale, 50);
g_pSettings->Key("local.function_status", cvars::visuals.local_function_status);
g_pSettings->Key("local.hotkey_list", cvars::visuals.local_hotkey_list);
g_pSettings->Key("local.statistics", cvars::visuals.local_statistics);
g_pSettings->Key("local.statistics.hits", cvars::visuals.local_statistics_elements[0], true);
g_pSettings->Key("local.statistics.misses", cvars::visuals.local_statistics_elements[1], true);
g_pSettings->Key("local.statistics.percent", cvars::visuals.local_statistics_elements[2], true);
g_pSettings->Key("local.bullet_tracers", cvars::visuals.local_bullet_tracers);
g_pSettings->Key("local.bullet_tracers.time", cvars::visuals.local_bullet_tracers_time, 1.f);
g_pSettings->Key("local.bullet_tracers.color", cvars::visuals.local_bullet_tracers_color, "64,255,64,255");
g_pSettings->Key("effects.custom.color_hud", cvars::visuals.effects_custom_color_hud);
g_pSettings->Key("effects.custom.color_hud.color", cvars::visuals.effects_custom_color_hud_color, "0,255,0,255");
g_pSettings->Key("effects.screen_fade_limit", cvars::visuals.effects_screen_fade_limit, 75.f);
g_pSettings->Key("effects.color_filter", cvars::visuals.effects_color_filter);
g_pSettings->Key("effects.color_filter.color", cvars::visuals.effects_color_filter_color, "255,255,255,255");
g_pSettings->Key("effects.poly_filter", cvars::visuals.effects_poly_filter);
...
static void FileKreedzHandle()
{
	...
	g_pSettings->Key("kreedz.key", cvars::kreedz.key);

	g_pSettings->Key("kreedz.bunnyhop", cvars::kreedz.bunnyhop);

	g_pSettings->Key("kreedz.groundstrafe", cvars::kreedz.groundstrafe);
	g_pSettings->Key("kreedz.groundstrafe.gain", cvars::kreedz.groundstrafe_gain, 1.f);

	g_pSettings->Key("kreedz.fastrun", cvars::kreedz.fastrun);
	g_pSettings->Key("kreedz.fastrun.key", cvars::kreedz.fastrun_key);

	g_pSettings->Key("kreedz.degen", cvars::kreedz.degen);
	g_pSettings->Key("kreedz.degen.key", cvars::kreedz.degen_key);
	g_pSettings->Key("kreedz.degen.tilt", cvars::kreedz.degen_tilt, 0.f);
	g_pSettings->Key("kreedz.degen.power", cvars::kreedz.degen_power, 400.f);

	g_pSettings->Key("kreedz.auto_jof", cvars::kreedz.auto_jof);
	g_pSettings->Key("kreedz.auto_jof.min_distance", cvars::kreedz.auto_jof_min_distance, 2.f);

	g_pSettings->Key("kreedz.jumpbug", cvars::kreedz.jumpbug);

	g_pSettings->Key("kreedz.edgebug", cvars::kreedz.edgebug);

	g_pSettings->Key("kreedz.wallbug", cvars::kreedz.wallbug);

	g_pSettings->Key("kreedz.slowwalk", cvars::kreedz.slowwalk);
	g_pSettings->Key("kreedz.slowwalk.key", cvars::kreedz.slowwalk_key);
	g_pSettings->Key("kreedz.slowwalk.speed", cvars::kreedz.slowwalk_speed, 34.f);

	g_pSettings->Key("kreedz.rage_strafe", cvars::kreedz.rage_strafe);
...
static void FileMiscHandle()
{
	...
	g_pSettings->Key("fakelatency.hold", cvars::misc.fakelatency_hold);
	g_pSettings->Key("fakelatency.amount", cvars::misc.fakelatency_amount, 200);
	g_pSettings->Key("fakelatency.key", cvars::misc.fakelatency_key);
	g_pSettings->Key("namestealer.enabled", cvars::misc.namestealer);
	g_pSettings->Key("namestealer.interval", cvars::misc.namestealer_interval, 5);
	g_pSettings->Key("replace_models_with_original", cvars::misc.replace_models_with_original, false);
	g_pSettings->Key("steamid_spoofer", cvars::misc.steamid_spoofer, false);
	g_pSettings->Key("auto_block.enabled", cvars::misc.auto_block);
	g_pSettings->Key("auto_block.key", cvars::misc.auto_block_key);
	g_pSettings->Key("auto_boost.enabled", cvars::misc.auto_boost);
	g_pSettings->Key("auto_boost.key", cvars::misc.auto_boost_key);
	g_pSettings->Key("predict.local", cvars::misc.predict_local, true);
	g_pSettings->Key("predict.players", cvars::misc.predict_players, true);
	g_pSettings->Key("motd.block", cvars::misc.motd_block, true);
	g_pSettings->Key("frame_skip.enabled", cvars::misc.frame_skip);
	g_pSettings->Key("frame_skip.amount", cvars::misc.frame_skip_amount, 1);
	g_pSettings->Key("maximize_on_respawn", cvars::misc.maximize_on_respawn);
	g_pSettings->Key("client_weapons", cvars::misc.client_weapons, true);
	g_pSettings->Key("sandbox.enabled", cvars::misc.sandbox_enabled, true);
	g_pSettings->Key("speed.enabled", cvars::misc.speed_enabled);
	g_pSettings->Key("speed.key", cvars::misc.speed_key);
	g_pSettings->Key("speed.value", cvars::misc.speed_value, 2.f);
	g_pSettings->Key("airstuck.enabled", cvars::misc.airstuck);
	g_pSettings->Key("airstuck.key", cvars::misc.airstuck_key);
	g_pSettings->Key("doubletap.enabled", cvars::misc.doubletap);
	g_pSettings->Key("doubletap.key", cvars::misc.doubletap_key);
	g_pSettings->Key("doubletap.shift", cvars::misc.doubletap_shift, 1);
	...
	g_pSettings->Key("knifebot.aim.hitbox_scale", cvars::misc.kb_aim_hitbox_scale, 100.f);
	...
	g_pSettings->Key("knifebot.debug", cvars::misc.kb_debug);
