...
void CHitRegister::Update(usercmd_s* cmd)
{
	...
	static bool prevattack, prevattack2;

	const bool bShotFired = (g_Weapon->m_iWeaponID == prevweaponid && g_Weapon->m_iClip < prevweaponclip);
	...
	if (cvars::visuals.local_bullet_tracers && !g_Local->m_bIsDead)
	{
		if (g_Weapon.IsGun() && bShotFired)
		{
		...
		else if (g_Weapon.IsKnife() && ((m_bIsInAttack && !prevattack) || (m_bIsInAttack2 && !prevattack2)))
		{
			...
			QViewAngles.AngleVectors(&vecDir, NULL, NULL);
		...
		while (g_BulletTracers.size() > 32)
			g_BulletTracers.pop_front();
	}

	prevattack = m_bIsInAttack;
	prevattack2 = m_bIsInAttack2;
