...
enum WeaponTablesList { GLOBAL, PISTOL, SMG, RIFLE, SHOTGUN, SNIPER, MAX_WEPON_GROUPS };
...
void WeaponCombo(const std::unique_ptr<CMenuGui>& m_pGui, int selected_weapon_group, int* selected_weapon)
{
	...
	{
	case GLOBAL:	m_pGui->Combo(selected_weapon, "##weapon_group", weapons_global, IM_ARRAYSIZE(weapons_global), "", true); break;
...
{
	// "Global" weapon group or the first (shared) combo entry -> shared config slot cvars::weapons[WEAPON_NONE]
	if (selected_weapon_group == GLOBAL || selected_weapon == 0)
	{
		*current_weapon_id = WEAPON_NONE;
		return;
	}
...
{
	// "Apply to all" edits the shared Global slot when it is selected
	if (selected_weapon_group == GLOBAL || current_weapon_id == WEAPON_NONE)
	{
		for (int i = 1; i < WEAPON_MAX_COUNT; i++)
			if (WeaponFilter(i))
				cvars::weapons[i] = cvars::weapons[WEAPON_NONE];
...
void CMenu::DrawLegacy()
{
	...
	enum VisualsTablesList { VISUALS_MAIN, VISUALS_ESP, VISUALS_CHAMS, VISUALS_OTHER, VISUALS_COLORS };
	...
	{
		weapon_tables_data.push_back(TabWidgetsData{ "GLOBAL", 0.f });
	...
	{
		visuals_tables_data.push_back(TabWidgetsData{ "MAIN", 0.f });
	...
	if (m_pGui->Begin("##menu", ImVec2(876, 623)))
	{
		m_WindowPos = GImGui->CurrentWindow->Pos;
		m_WindowSize = GImGui->CurrentWindow->Size;
		m_WindowCanvasPos = ImVec2(m_WindowPos.x + 146.f, m_WindowPos.y + 56.f);
		m_WindowCanvasSize = ImVec2(m_WindowSize.x - 146.f, m_WindowSize.y - 140.f);
		...
		case LEGIT:
		{
			...
			const float legit_col_w = child_size.x;
			...
			// Top-middle: weapon class tabs (PISTOL/SMG/...). Keep single column width.
			child_size.x = legit_col_w;
			...
			// 3 columns row below top bar.
			m_pGui->SetChildSize(child_size);
			child_size.x = legit_col_w;
			const float legit_cols_h = child_size.y;

			ImGui::BeginGroup(); // left column

			child_size.y = (legit_cols_h - WINDOW_PADDING_SAMELINE) * 0.5f;
			child_size.x = legit_col_w;
			...
			child_size.y = legit_cols_h - child_size.y - WINDOW_PADDING_SAMELINE - 3.f;
			child_size.x = legit_col_w;
			...
			m_pGui->Slider(&cvars::legitbot.trigger_shot_delay, "Shot delay", 0, 1000, "%ims"); m_pGui->Checkbox(&cvars::legitbot.trigger_dynamic, "Dynamic"); m_pGui->Checkbox(&cvars::legitbot.trigger_turn_off_after_first_bullet, "Turn off after 1st bullet");
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();
			...
			ImGui::SameLine(0, WINDOW_PADDING_SAMELINE);
			ImGui::BeginGroup(); // middle column

			child_size.y = legit_cols_h;
			child_size.x = legit_col_w;

			const char* aim_main_selection[] = { "Closest by FOV", "Hitbox weights" };
			...
			// Middle column top: general aim settings (was "Aimbot main" in left column,
			// moved here so columns don't overlap).
			...
			m_pGui->Checkbox(&cvars::legitbot.aim_smoke_check, "Smoke check");
			m_pGui->Slider(&cvars::legitbot.aim_flashed_check, "Flashed check", 0, 100, "%i%%");
			m_pGui->Checkbox(&cvars::legitbot.aim_shoot_through_teammates, "Shoot through teammates");
			m_pGui->Checkbox(&cvars::legitbot.aim_pseudo_professional, "Pseudo professional");
			m_pGui->Checkbox(&cvars::legitbot.aim_write_demo_visible_only, "Write in demo only visible angles");
			m_pGui->Checkbox(&cvars::legitbot.aim_mouse_event, "Mouse event aiming");
			m_pGui->Checkbox(&cvars::legitbot.aim_fov_scale_by_fps, "FOV scale by game FPS");
			m_pGui->Checkbox(&cvars::legitbot.aim_dynamic_fov, "Dynamic FOV");
			m_pGui->Combo(&cvars::legitbot.target_aim_selection, "Aim selection", aim_main_selection, IM_ARRAYSIZE(aim_main_selection));
			m_pGui->Slider(&cvars::legitbot.target_weight_head, "Head weight", 1, 100, "%i");
			m_pGui->Slider(&cvars::legitbot.target_weight_neck, "Neck weight", 1, 100, "%i");
			m_pGui->Slider(&cvars::legitbot.target_weight_chest, "Chest weight", 1, 100, "%i");
			m_pGui->Slider(&cvars::legitbot.target_weight_stomach, "Stomach weight", 1, 100, "%i");
			m_pGui->Slider(&cvars::legitbot.target_weight_arms, "Arms weight", 1, 100, "%i");
			m_pGui->Slider(&cvars::legitbot.target_weight_legs, "Legs weight", 1, 100, "%i");
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();
			...
			// Middle column: full row height, content scrolls inside.
			...
			m_pGui->Checkbox(&cvars::legitbot.aim_recoil_return_angles, "Recoil return angles");
			m_pGui->Checkbox(&cvars::legitbot.aim_recoil_no_mousemove_trigger, "Recoil: no mouse move trigger");
			m_pGui->Checkbox(&cvars::legitbot.aim_recoil_disable_smooth_auto, "Recoil: disable smooth auto");
			...
			ImGui::EndGroup(); // end middle column group
		...
		case RAGE:
		{
			...
			// Shared with the WEAPON sub-tab, so per-weapon widgets in MAIN follow the selected weapon.
			static int weapon_tables_selected, weapon_tables_hovered;
			static int current_weapon_id, selected_weapon[MAX_WEPON_GROUPS];
			static bool auto_change_current_weapon;
			...
			case RAGE_MAIN:
			{
				...
				// Per-weapon widgets below follow the weapon selected in the WEAPON sub-tab.
				GetCurrentWeaponId(weapon_tables_selected, selected_weapon[weapon_tables_selected], &current_weapon_id);
				...
				m_pGui->Key(&cvars::ragebot.raim_key, "Key");
				...
				m_pGui->Slider(&cvars::ragebot.raim_hitchance_seeds, "Hitchance", 1, 256, "%i");
				m_pGui->Slider(&cvars::weapons[current_weapon_id].raim_auto_penetration_min_damage, "Minimum damage", 1, 100, "%i");
				m_pGui->Key(&cvars::ragebot.raim_force_min_damage_key, "Minimum damage (override)");
				m_pGui->Slider(&cvars::ragebot.raim_force_min_damage_value, "Minimum damage value", 1, 100, "%i");
				...
				case RAGE_ANTIAIM_MAIN:
				{
					...
					m_pGui->Combo(&cvars::ragebot.aa_roll, "Roll", aa_roll, IM_ARRAYSIZE(aa_roll), "It works only on special servers.");
			...
			case RAGE_WEAPON:
			{
				...
				m_pGui->Checkbox(&cvars::weapons[current_weapon_id].raim_auto_penetration, "Auto penetration"); m_pGui->Slider(&cvars::weapons[current_weapon_id].raim_hitchance_seeds, "Hitchance seeds", 0, 256, "%i"); m_pGui->Slider(&cvars::weapons[current_weapon_id].raim_force_min_damage, "Force min. damage (override, 0=global)", 0, 100, "%i");
		...
		case VISUALS:
		{
			...
			case VISUALS_MAIN:
			{
				...
				m_pGui->BeginGroupBox("General", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##visuals_main_general");
				...
				m_pGui->BeginGroupBox("Menu", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##visuals_main_menu");
				m_pGui->Checkbox(&cvars::visuals.main_allow_resize, "Allow resize", "Allow the menu window to be resized.");
				m_pGui->Checkbox(&cvars::visuals.main_notifications, "Notifications", "Display notifications.");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				m_pGui->BeginGroupBox("Watermark", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##visuals_main_watermark");
				m_pGui->Checkbox(&cvars::visuals.watermark, "Enabled");
				m_pGui->Checkbox(&cvars::visuals.watermark_credits, "Credits", "Always display the small authorship mark in the corner of the screen.");
				static const char* watermark_elements[] = { "Time", "Name", "Framerate", "Latency" };
				m_pGui->MultiCombo(cvars::visuals.watermark_elements, "Elements", watermark_elements, IM_ARRAYSIZE(watermark_elements));
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##visuals_main_other");
				m_pGui->Checkbox(&cvars::visuals.shared_esp, "Shared ESP");
			...
			case VISUALS_OTHER:
			{
				...
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##visuals_other_crosshair");
				m_pGui->Checkbox(&cvars::visuals.crosshair_enabled, "Enabled"); m_pGui->Checkbox(&cvars::visuals.crosshair_dot, "Dot"); m_pGui->Slider(&cvars::visuals.crosshair_size, "Size", 1, 40, "%i"); m_pGui->Slider(&cvars::visuals.crosshair_gap, "Gap", 0, 30, "%i"); m_pGui->Slider(&cvars::visuals.crosshair_thickness, "Thickness", 1, 10, "%i"); m_pGui->Checkbox(&cvars::visuals.crosshair_dynamic, "Dynamic"); m_pGui->Slider(&cvars::visuals.crosshair_dynamic_scale, "Dynamic scale", 0, 100, "%i%%");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				{	
					...
					m_pGui->Checkbox(&cvars::visuals.esp_other_local_toggle_status, "Toggle status"); m_pGui->Checkbox(&cvars::visuals.local_function_status, "Function status"); m_pGui->Checkbox(&cvars::visuals.local_hotkey_list, "Hotkey list"); m_pGui->Checkbox(&cvars::visuals.local_statistics, "Statistics"); static const char* stat_elements[] = { "Hits", "Misses", "%" }; m_pGui->MultiCombo(cvars::visuals.local_statistics_elements, "Statistics elements", stat_elements, 3); m_pGui->Checkbox(&cvars::visuals.local_bullet_tracers, "Bullet tracers"); m_pGui->Slider(&cvars::visuals.local_bullet_tracers_time, "Bullet tracers time", 0.1f, 5.f, "%.1f sec", 1.f); m_pGui->ColorEdit(cvars::visuals.local_bullet_tracers_color, "Bullet tracers color");	
					break;
				}
				...
				m_pGui->Checkbox(&cvars::visuals.remove_scope, "Remove scope"); m_pGui->Checkbox(&cvars::visuals.remove_visual_recoil, "Remove visual recoil"); m_pGui->Checkbox(&cvars::visuals.remove_smoke, "Remove smoke"); m_pGui->Checkbox(&cvars::visuals.remove_screenshake, "Remove screenshake"); m_pGui->Checkbox(&cvars::visuals.remove_events, "Remove events");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				m_pGui->Slider(&cvars::visuals.effects_custom_render_fov, "Custom render FOV", 90, 150, "%i"); m_pGui->Checkbox(&cvars::visuals.effects_custom_color_hud, "Custom color HUD"); m_pGui->Slider(&cvars::visuals.effects_screen_fade_limit, "Screen fade limit", 0.f, 100.f, "%.0f%%", 1.f); m_pGui->Checkbox(&cvars::visuals.effects_color_filter, "Color filter"); m_pGui->Checkbox(&cvars::visuals.effects_poly_filter, "Poly filter");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
		...
		case KREEDZ:
		{
			...
			m_pGui->BeginWidgetsGroup();
			m_pGui->SetID("##kreedz_main");
			m_pGui->Checkbox(&cvars::kreedz.active, "Enabled", "Global on/off of the kreedz stuff.");
			m_pGui->Key(&cvars::kreedz.key, "Key");
			m_pGui->Checkbox(&cvars::kreedz.bunnyhop, "Bunnyhop", "Jumps on the exact frame you touch the ground.");
			m_pGui->Checkbox(&cvars::kreedz.groundstrafe, "Ground strafe", "Circle-strafe steering on the ground: pays friction along the velocity and spends the rest of the accel on the turn. Run AFTER FastRun in the pipeline so it keeps the final word on move.");
			m_pGui->Slider(&cvars::kreedz.groundstrafe_gain, "GS gain", 0.f, 1.f, "%.2f", 0.01f);
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();
			...
			m_pGui->BeginGroupBox("Bugs", child_size);
			m_pGui->BeginWidgetsGroup();
			m_pGui->SetID("##kreedz_bugs");
			m_pGui->Checkbox(&cvars::kreedz.jumpbug, "Jump bug", "Duck + jump on the landing frame so no fall damage is applied.");
			m_pGui->Checkbox(&cvars::kreedz.edgebug, "Edge bug", "Duck before touching a ledge to land on the edge.");
			m_pGui->Checkbox(&cvars::kreedz.wallbug, "Wall bug", "Duck against a wall while airborne to keep the velocity.");
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();
			...
			m_pGui->BeginGroupBox("Strafes", child_size);
			m_pGui->BeginWidgetsGroup();
			m_pGui->SetID("##kreedz_strafes");
			m_pGui->Checkbox(&cvars::kreedz.rage_strafe, "Rage strafe", "Solves the air wishdir to the perpendicular every tick (max gain) and snaps the view onto the velocity. No more circling.");
			m_pGui->Checkbox(&cvars::kreedz.legit_strafe, "Legit strafe", "Same solved wishdir, but eases the view onto the velocity instead of snapping.");
			m_pGui->Slider(&cvars::kreedz.legit_strafe_speed, "Legit strafe speed", 1.f, 100.f, "%.0f%%", 1.f);
			m_pGui->Checkbox(&cvars::kreedz.fastrun, "Fast run", "Tkz 277 u/s grounded walk boost.");
			m_pGui->Key(&cvars::kreedz.fastrun_key, "Fast run key");
			m_pGui->Checkbox(&cvars::kreedz.auto_jof, "Auto jump of fall", "Auto-jumps exactly at the ledge so no fall damage.");
			m_pGui->Slider(&cvars::kreedz.auto_jof_min_distance, "JOF min distance", 1.f, 16.f, "%.0f units", 1.f);
			m_pGui->Checkbox(&cvars::kreedz.degen, "DeGen", "Solves the air wishdir to be exactly perpendicular to the velocity every tick - constant max gain, no view sync needed.");
			m_pGui->Key(&cvars::kreedz.degen_key, "DeGen key");
			m_pGui->Slider(&cvars::kreedz.degen_power, "DeGen power", 50.f, 1000.f, "%.0f units", 1.f);
			m_pGui->Slider(&cvars::kreedz.degen_tilt, "DeGen tilt", -0.9f, 0.9f, "%.2f", 0.01f);
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();

			ImGui::Spacing(3);

			m_pGui->BeginGroupBox("Slowwalk", child_size);
			m_pGui->BeginWidgetsGroup();
			m_pGui->SetID("##kreedz_slowwalk");
			m_pGui->Checkbox(&cvars::kreedz.slowwalk, "Enabled");
			m_pGui->Key(&cvars::kreedz.slowwalk_key, "Key");
			m_pGui->Slider(&cvars::kreedz.slowwalk_speed, "Speed", 1.f, 250.f, "%.0f units", 1.f);
			m_pGui->EndWidgetsGroup();
			m_pGui->EndGroupBox();
		...
		case MISC:
		{
			...
			//{
				...
				m_pGui->Checkbox(&cvars::misc.kb_debug, "Debug", "Logs every gate of the knifebot to the cheat console (stage=, clip=, trace_index=)");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##misc_speedhack");
				m_pGui->Checkbox(&cvars::misc.speed_enabled, "Enabled", "Client-side host timescale emulation, only applied while the key is held.");
				m_pGui->Slider(&cvars::misc.speed_value, "Speed", 0.1f, 5.f, "%.1fx", 1.f);
				m_pGui->Key(&cvars::misc.speed_key, "Key");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();

				ImGui::Spacing(3);

				m_pGui->BeginGroupBox("Exploits", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##misc_exploits");
				m_pGui->Checkbox(&cvars::misc.doubletap, "Doubletap", "Chokes the packet and resends the command so two shots leave in one tick.");
				m_pGui->Slider(&cvars::misc.doubletap_shift, "Doubletap shift", 1, 4, "%i");
				m_pGui->Key(&cvars::misc.doubletap_key, "Doubletap key");
				m_pGui->Checkbox(&cvars::misc.airstuck, "Airstuck", "Chokes the packets while the key is held.");
				m_pGui->Key(&cvars::misc.airstuck_key, "Airstuck key");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
				...
				m_pGui->Checkbox(&cvars::misc.nospread, "No spread", "Compensates weapon spread via predicted random seed.");
				static const char* nospread_modes[] = { "Pitch / Yaw", "Pitch / Roll", "Pitch / Yaw / Roll" };
				m_pGui->Combo(&cvars::misc.nospread_mode, "Spread mode", nospread_modes, IM_ARRAYSIZE(nospread_modes));
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();

				ImGui::Spacing(3);

				m_pGui->BeginGroupBox("Performance", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##misc_performance");
				m_pGui->Checkbox(&cvars::misc.fps_unlock, "Unlock FPS", "Forces fps_max 999 locally. Server cvar checks still get answered with 100.");
				m_pGui->Checkbox(&cvars::misc.fps_developer, "Force developer 0", "Removes the FPS drop caused by developer 1.");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();

				ImGui::Spacing(3);

				m_pGui->BeginGroupBox("Extras", child_size);
				m_pGui->BeginWidgetsGroup();
				m_pGui->SetID("##misc_extras");
				m_pGui->Checkbox(&cvars::misc.auto_block, "Auto block", "Uses a teammate as cover on the block key.");
				m_pGui->Key(&cvars::misc.auto_block_key, "Auto block key");
				m_pGui->Checkbox(&cvars::misc.auto_boost, "Auto boost", "Ducks to boost a teammate on the boost key.");
				m_pGui->Key(&cvars::misc.auto_boost_key, "Auto boost key");
				m_pGui->Checkbox(&cvars::misc.motd_block, "Block MOTD", "Keeps the MOTD window from opening.");
				m_pGui->Checkbox(&cvars::misc.frame_skip, "Frame skip", "Skips the expensive visual frame work to raise the FPS.");
				m_pGui->Slider(&cvars::misc.frame_skip_amount, "Frame skip amount", 1, 10, "%i");
				m_pGui->Checkbox(&cvars::misc.maximize_on_respawn, "Maximize on respawn", "Restores the game window when you respawn.");
				m_pGui->Checkbox(&cvars::misc.client_weapons, "Client-side weapons", "Forces the weapon prediction (cl_lw).");
				m_pGui->Checkbox(&cvars::misc.predict_local, "Predict local", "Forces the lag compensation (cl_lc).");
				m_pGui->Checkbox(&cvars::misc.predict_players, "Predict players", "Forces the weapon prediction (cl_lw).");
				m_pGui->Checkbox(&cvars::misc.sandbox_enabled, "Sandbox", "Answers the server cvar checks with the stock values.");
				m_pGui->EndWidgetsGroup();
				m_pGui->EndGroupBox();
		...
		case CONFIGS:
		{
			...
			if (ImGui::Button("Save##settings", button_size) || m_pHotkeys->Is("CTRL + S"))
