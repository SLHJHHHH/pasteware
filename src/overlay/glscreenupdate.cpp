...
void HandleGLSceneUpdate()
{
	...
	const bool bSkipFrame = g_pMiscellaneous.get() && g_pMiscellaneous->ShouldSkipFrame();

	if (GImGui && !Game::IsTakenScreenshot())
	{
		if (Game::IsConnected() && !bSkipFrame && cvars::visuals.streamer_mode && !g_pIGameUI->IsGameUIActive())
		{
		...
		{
			if (Game::IsConnected() && !IS_NULLPTR(g_pVisuals) && !bSkipFrame)
				g_pVisuals->Overlay();
		...
		if (cvars::visuals.copyright && !Game::IsTakenScreenshot())
			DrawCopyright();
