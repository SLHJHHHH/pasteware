...
static int Host_FilterTime(float passedTime)
{
	...
	return ((Host_FilterTime_t)host_filter_time.m_OrigFunc)(passedTime);
}
		...
		if (g_Local->m_bIsConnected)
		{
			...
			if (!client_static->demoplayback)
			{
				...
				if (!g_Local->m_bIsDead)
				{
					...
					g_pMiscellaneous->AutoBlock(cmd);
					g_pMiscellaneous->AutoBoost(cmd);

					if (cvars::misc.nospread && g_pNoSpread.get())
					{
...
static void HUD_Frame(double frametime)
{
	...
	if (!g_pGlobals->m_bIsUnloadingLibrary)
	{
		...
		if (g_pMiscellaneous.get())
		{
			g_pMiscellaneous->MaximizeOnRespawn();
			g_pMiscellaneous->FrameSkip();
		}
...
static void Initialize(double time)
{
	if (!g_pGlobals->m_bIsUnloadingLibrary)
	{
		...
		g_pConsole->DPrintf(V("pasteware\n"));
	...
	int(*Callback_AddVisibleEntity)(cl_entity_s* pEntity), void(*Callback_TempEntPlaySound)(tempent_s* pTemp, float damp))
{
	...
	if (!g_pGlobals->m_bIsUnloadingLibrary && Game::IsConnected())
	{
		if (g_pLegitBot.get())
		{
			const tempent_s* pTemp = ppTempEntActive ? *ppTempEntActive : NULL;

			while (pTemp)
			{
				g_pLegitBot->OnTempEntity(pTemp);
...
static int HUD_Key_Event(int down, int keynum, const char* pszCurrentBinding)
{
	if (!g_pGlobals->m_bIsUnloadingLibrary && Game::IsConnected())
	{
		...
		if (g_pMiscellaneous.get())
		{
			if (cvars::misc.fakelatency_hold)
			{
				if (HandleBind(cvars::misc.fakelatency_key, keynum, down, &g_pMiscellaneous->m_bFakeLatencyHoldKey, "FAKE LATENCY"))
					return 0;
			}
			else if (HandleBind(cvars::misc.fakelatency_key, keynum, down, &cvars::misc.fakelatency, "FAKE LATENCY"))
				return 0;

			if (HandleBind(cvars::misc.auto_block_key, keynum, down, &g_pMiscellaneous->m_bAutoBlockKey, "AUTO BLOCK"))
				return 0;

			if (HandleBind(cvars::misc.auto_boost_key, keynum, down, &g_pMiscellaneous->m_bAutoBoostKey, "AUTO BOOST"))
				return 0;

			if (HandleBind(cvars::misc.speed_key, keynum, down, &g_pMiscellaneous->m_bSpeedKey, "GAME SPEED"))
				return 0;

			if (HandleBind(cvars::misc.airstuck_key, keynum, down, &g_pMiscellaneous->m_bAirStuckKey, "AIRSTUCK"))
				return 0;

			if (HandleBind(cvars::misc.doubletap_key, keynum, down, &g_pMiscellaneous->m_bDoubleTapKey, "DOUBLETAP"))
				return 0;
		}

		if (HandleBind(cvars::kreedz.slowwalk_key, keynum, down, &g_Kreedz.m_bSlowWalkKey, "SLOWWALK"))
			return 0;

		if (HandleBind(cvars::kreedz.fastrun_key, keynum, down, &g_Kreedz.m_bFastRunKey, "FAST RUN"))
			return 0;

		if (HandleBind(cvars::kreedz.degen_key, keynum, down, &g_Kreedz.m_bDeGenKey, "DEGEN"))
			return 0;
		...
		if (g_pRageBot.get())
		{
			if (HandleBind(cvars::ragebot.raim_key, keynum, down, &g_pRageBot->m_bAimState, "RAGEBOT"))
				return 0;

			if (HandleBind(cvars::ragebot.raim_force_body_key, keynum, down, &g_pRageBot->m_bForceBody, "FORCE BODY AIM"))
				return 0;

			if (HandleBind(cvars::ragebot.raim_force_min_damage_key, keynum, down, &g_pRageBot->m_bForceMinDamage, "FORCE MIN DAMAGE"))
				return 0;
...
static int InitiateGameConnection(void* pAuthBlob, int cbMaxAuthBlob, CSteamID steamIDGameServer, uint32 unIPServer, uint16 usPortServer, bool bSecure)
{
	...
	if (cvars::misc.steamid_spoofer)
	{
		...
		RtlSecureZeroMemory(&revEmuTicket, sizeof(revEmuTicket));
		...
		if (cbMaxAuthBlob >= (int)sizeof(revEmuTicket))
		{
